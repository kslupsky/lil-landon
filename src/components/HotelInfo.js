...
const HotelInfo = () => {
  ...
  const loadAccessibilityData = async () => {
    // Query API Gateway
    const resp = await fetch('https://8xm31hgecd.execute-api.us-east-1.amazonaws.com/Production/accessibility');
  ...
  const loadServicesData = async () => {
    // Query API Gateway
    const resp = await fetch('https://8xm31hgecd.execute-api.us-east-1.amazonaws.com/Production/services');
    let jsonData = await resp.json();
    // Assign the response data to the state variable
